...
class mono(nn.Module):
    def __init__(self, device=torch.device('cuda:0'),
        ...
        self.kk = [
            ...
            [0., 0., 1.]]  # camera intrinsic for CSRD
        # self.kk = [[886.8100, 0.000000, 512.0],
        ...
        # Load Models
        self.net = Loco(
            model=dic_models[args.mode],  # models/pretrained_loconet.pth
            device=device,
            n_dropout=args.n_dropout,
            p_dropout=args.dropout,
            without_load_model=without_load_model)  # 5view: without_load_model=True
...
class Loco(torch.nn.Module):
    ...
    def forward(self, keypoints, kk, boxes):
        ...
        inputs = preprocess_monoloco(keypoints, kk)  # （m，34），m是目标数量, 34是（x，y，x，y，……），每个目标的骨架点坐标（相机坐标系下）

        outputs = self.model(inputs)  # inputs的目标数量m可以当做batchsize，outputs（m，9）
    ...
    def epistemic_uncertainty(self, inputs):
        ...
        for _ in range(self.n_dropout):
            ...
            if self.net == 'monoloco_test_CSRD':
                db = outputs[:, 0:2]
            else:
                db = outputs[:, 2:4]
    ...
    def post_process(dic_in, boxes, keypoints, kk, dic_gt=None, iou_min=0.3, reorder=True, verbose=False):
        """Post process monoloco_test_CSRD to output final dictionary with all information for visualizations"""
        if len(boxes.shape) == 1:
            boxes = boxes.unsqueeze(dim=0)
        dic_out = defaultdict(list)  # 创建默认字典，当访问的键不存在时，会自动创建该键，并将其值初始化为一个空list
        if dic_in is None:
            return dic_out
        ...
        if dic_gt:
            ...
            matches = get_iou_matches(boxes, boxes_gt, iou_min=iou_min)  # 计算pifpaf预测的bbox和真值bbox的匹配结果
            dic_out['gt'] = [True]
            if verbose:
        ...
        uv_shoulders = get_keypoints(keypoints, mode='shoulder')  # (m, 2) 肩膀点的平均点
        uv_heads = get_keypoints(keypoints, mode='head')  # (m, 2)
        uv_centers = get_keypoints(keypoints, mode='center')  # (m, 2)
        xy_centers = pixel_to_camera(uv_centers, kk, 1)  # (m, 3)，这里的相机坐标系下的坐标，假定深度距离是1（其实是未知）

        # Add all the predicted annotations, starting with the ones that match a ground-truth
        for idx in all_idxs:  # 遍历视角中的所有行人
            kps = keypoints[idx]
            box = boxes[idx]
            ...
            xyz_pred = xyz_from_distance(dd_pred, torch.tensor(xy_centers[idx], device=dd_pred.device))[0]  # 根据预测的深度d计算实际空间坐标
            distance = math.sqrt(float(xyz_pred[0]) ** 2 + float(xyz_pred[1]) ** 2 + float(xyz_pred[2]) ** 2)  # 该值应当与dd_pred相同
            conf = 0.035 * (box[-1]) / (bi / distance)  # 根据预测距离的不确定性调整bbox框的置信度

            dic_out['boxes'].append(box)  # 存相机视角下，所有行人的bbox框
            dic_out['confs'].append(conf)  # 存调整后的行人bbox框的置信度
            dic_out['dds_pred'].append(dd_pred)  # 存每个行人的预测距离
            dic_out['stds_ale'].append(bi)  # 预测距离的不确定性，Aleatoric Uncertainty（偶然不确定性）
            dic_out['stds_epi'].append(var_y)  # 预测距离标准差列表，Epistemic Uncertainty（认知不确定性）

            # dic_out['xyz_pred'].append(xyz_pred.squeeze().tolist())
            dic_out['xyz_pred'].append(xyz_pred.squeeze())  # 模型预测的实际空间的行人三维坐标
            dic_out['uv_kps'].append(kps)  # 存每个行人的关键点
            dic_out['uv_centers'].append(uv_center)  # 中心点列表
            dic_out['uv_shoulders'].append(uv_shoulder)  # 肩点列表
            dic_out['uv_heads'].append(uv_head)  # 头点列表
            ...
            try:  # 水平向右为0度
                ...
                dic_out['angles'].append((dic_in['yaw'][0][idx]))  # Predicted angle 模型直接预测的朝向角，弧度
                dic_out['angles_egocentric'].append((dic_in['yaw'][1][idx]))  # Egocentric angle 根据模型预测的球坐标系下的距离和角度等，校正的朝向角，并限制在[-pi, pi]之间
            except KeyError:
                continue
...
class LocoModel(nn.Module):
    ...
    def forward(self, x):
        ...
        return y  # 0、1、2是球坐标系下的预测


class MyLinearSimple(nn.Module):  # LocoNet中间×2的那个模块，但1024通道
...
def preprocess_monoloco(keypoints, kk, zero_center=False):
    ...
    uv_center = get_keypoints(keypoints, mode='center')  # (m, 2), 根据骨架点返回每个目标的中心坐标
    xy1_center = pixel_to_camera(uv_center, kk, 10)  # (m, 3), 3表示XYZ, 像素坐标转相机坐标
    xy1_all = pixel_to_camera(keypoints[:, 0:2, :], kk, 10)  # (m, 17, 3), 3表示XYZ, 假定距离都是10米，返回相机坐标系下的骨架点坐标
    if zero_center:
    ...
    return kps_out  # （m，34），34是（x，y，x，y，……），每个目标的骨架点坐标（相机坐标系下）
...
def pixel_to_camera(uv_tensor, kk, z_met):  # 像素坐标转相机坐标
    """
    Convert a tensor in pixel coordinate to absolute camera coordinates
    It accepts lists or torch/numpy tensors of (m, 2) or (m, 2, 17)
    ...
    # 检查矩阵是否可逆
    det = torch.det(kk.cpu())
    if det == 0:
        raise ValueError("Matrix is singular and cannot be inverted. Determinant is zero.")

    # 将矩阵移动到 CPU 进行求逆操作，然后移回原设备
    kk_1 = torch.linalg.inv(kk.cpu()).to(kk.device)
    # kk_1 = torch.inverse(kk)

    xyz_met_norm = torch.matmul(uv_padded, kk_1.t())  # More general than torch.mm，像素坐标系转归一化相机坐标系
    xyz_met = xyz_met_norm * z_met  # 乘以z，归一化相机坐标系转真实世界坐标系
...
def xyz_from_distance(distances, xy_centers):
    ...
    根据预测的深度d计算实际空间坐标：
    预测深度相当于针孔成像模型中，原点到目标点的距离
    将相机坐标系下的坐标（z=1）成比例缩放得到预测的真实空间坐标
    """
...
def extract_outputs(outputs, tasks=()):  # outputs (m, 9)
    dic_out = {'x': outputs[:, 0:1],  # 球坐标系下的theta
               'y': outputs[:, 1:2],  # 球坐标系下的phi
               'd': outputs[:, 2:4],  # 2是球坐标系下的r（即预测距离），3是类似置信度的值，用于计算标不确定性
               'h': outputs[:, 4:5],
               'w': outputs[:, 5:6],
               'l': outputs[:, 6:7],
               'ori': outputs[:, 7:9]}  # 8可能是LocoModel中单独拼接的朝向角

    bi = unnormalize_bi(dic_out['d'])
    dic_out['bi'] = bi  # 可能：预测距离的不确定性之类的

    x = to_cartesian(outputs[:, 0:3], mode='x')  # outputs的0、1、2分别表示球坐标系下的theta、phi、r
    y = to_cartesian(outputs[:, 0:3], mode='y')
    d = dic_out['d'][:, 0:1]  # 球坐标系的r
    z = torch.sqrt(d ** 2 - x ** 2 - y ** 2)
    dic_out['xyzd'] = torch.cat((x, y, z, d), dim=1)  # 三维坐标系的x、y、z、球坐标系的径向距离r）
    dic_out.pop('d')
    dic_out.pop('x')
    dic_out.pop('y')
    dic_out['d'] = d  # 预测的距离，即针孔成像模型中，原点到目标点的距离

    yaw_pred = torch.atan2(dic_out['ori'][:, 0:1], dic_out['ori'][:, 1:2])  # (m, 1) 用outputs的7、8列计算出yaw，模型直接预测的朝向角
    yaw_orig = back_correct_angles(yaw_pred, dic_out['xyzd'][:, 0:3])  # (m, 1) 根据模型预测的球坐标系下的距离和角度等，校正的朝向角，并限制在[-pi, pi]之间
...
def back_correct_angles(yaws, xyz):
    ...
    yaws[mask_down] += 2 * math.pi  # 限制在-pi到pi之间
