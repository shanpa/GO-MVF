...
class Config(object):
    ...
    def __init__(self, dataset_name, json_path='config/config.json'):
        ...
        if 'annotation' in config_file['Datasets'][dataset_name].keys():
            if 'CSRDR' in dataset_name and 'tracking' in dataset_name:
                ...
                try:
                    self.fv_dict = torch.load(config_file['Datasets'][dataset_name]['annotation'] + '/fv.pth')  # fv_dict[f"{frame}_{view_id}"].append([pid, bbox])
                except:
                    LOG.info('self.fv_dict path error, import absolute path /home/nv/ws/SwarmSee/SwarmSee-master/data/CSRD/virtual/annotation/fv.pth')
                    # self.fv_dict = torch.load('/home/nv/ws/SwarmSee/SwarmSee-master/data/CSRD/virtual/annotation/fv.pth')  # fv_dict[f"{frame}_{view_id}"].append([pid, bbox])
                    raise SystemExit("program end because of fv_dict path error")

        self.label = config_file['Datasets'][dataset_name]['label']  # annotation dir path
        ...
        # self.monoloco_suffix = '.monoloco_test_CSRD.json'
    ...
    def init_config(self, need_new_folder=True):
        ...
        self.result_path = os.path.join(self.output_path, self.exp_name)  # log path
        self.log_path = os.path.join(self.output_path, '%s/log.txt' % self.exp_name)  # log.txt path
