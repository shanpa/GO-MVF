...
def draw_predictions(xyr_list, id_list, frame_str, cfg, min_distance=0.25):
    """
    绘制俯视图箭头图，避免重叠
    :param xyr_list: 列表，每个元素是[x, y, angle(弧度)]
    :param id_list: 列表，每个元素是id (1为黑色，-1为蓝色)
    :param frame_str: 字符串，用于保存文件名
    :param min_distance: 最小允许的距离，小于此距离的箭头将被跳过
    """
    plt.figure(figsize=(5, 5))
    plt.gca().set_facecolor('white')

    # 创建图例用的代理对象
    black_patch = plt.Line2D([0], [0], marker='o', color='black', label='Target',
                             markerfacecolor='none', markersize=10, linestyle='None')
    blue_patch = plt.Line2D([0], [0], marker='o', color='blue', label='UGV',
                            markerfacecolor='none', markersize=10, linestyle='None')

    drawn_points = []  # 记录已绘制的点坐标

    for (x, y, r), pid in zip(xyr_list, id_list):
        # 检查是否与已绘制的点太近
        skip = False
        ...
        color = 'black' if pid == 1 else 'blue'
        dx = np.cos(-r) * 0.25
        dy = np.sin(-r) * 0.25

        # 先画空心圆点
        plt.scatter(x, y, facecolors='none', edgecolors=color, s=50)
        # 再画箭头
        plt.arrow(x, y, dx, dy,
                 head_width=0.15, head_length=0.2,
                 fc=color, ec=color, length_includes_head=True)
        drawn_points.append((x, y))  # 记录已绘制的点

    # 添加图例
    plt.legend(handles=[black_patch, blue_patch], loc='upper right')

    plt.xlim(-7.5, 7.5)
    plt.ylim(-2.5, 10)
    # plt.axis('equal')

    plt.savefig(os.path.join(cfg.result_path, f"figs/{frame_str}_2_white_background_top_view.png"), dpi=300, bbox_inches='tight')
    plt.close()

def draw_stack_predictions(bev_img_path, xyr_lists, id_list, frame_str, cfg, min_distance=0.25,
                           x_offset=0, y_offset=0, x_scale=1.0, y_scale=1.0):
    """
        绘制俯视图箭头图，避免重叠
        :param bev_img_path: 背景图像路径
        :param xyr_lists: 列表的列表，每个元素是[x, y, angle(弧度)]
        :param id_list: 列表，每个元素是id (1为黑色，-1为蓝色)
        :param frame_str: 字符串，用于保存文件名
        ...
        """
    # 读取背景图像
    img = plt.imread(bev_img_path)
    height, width = img.shape[:2]

    plt.figure(figsize=(5, 5))
    plt.imshow(img, extent=[0, width, 0, height])  # 显示背景图像
    plt.axis('off')  # 关闭坐标轴显示

    # 坐标转换参数 (将xyr坐标映射到图像像素坐标)
    x_min, x_max = -5, 5
    y_min, y_max = 0, 10
    ...
    # 创建图例用的代理对象
    black_patch = plt.Line2D([0], [0], marker='o', color='red', label='Target',
                             markerfacecolor='none', markersize=10, linestyle='None')
    blue_patch = plt.Line2D([0], [0], marker='o', color='blue', label='UGV',
                            markerfacecolor='none', markersize=10, linestyle='None')
    ...
    for i, xyr_list in enumerate(xyr_lists):
        color_ugv = cool_colors[i % len(cool_colors)]
        color_target = warm_colors[i % len(warm_colors)]
        drawn_points = []  # 记录已绘制的点坐标
        for (x, y, r), pid in zip(xyr_list, id_list):
            # 应用偏移和缩放
            ...
            # 将坐标转换为图像像素坐标
            img_x = (x - x_min) * scale_x
            img_y = (y - y_min) * scale_y

            # 检查是否与已绘制的点太近
            skip = False
            for (px, py) in drawn_points:
                if np.sqrt((img_x - px) ** 2 + (img_y - py) ** 2) < min_distance * min(scale_x, scale_y):
                    skip = True
                    break
            if skip:
                continue

            color = color_target if pid == 1 else color_ugv
            edge_color = (*(np.array(color) / 255), 0.7)  # 带透明度的边缘色
            face_color = (*(np.array(color) / 255), 0.3)  # 带透明度的填充色

            dx = np.cos(-r) * 25
            dy = np.sin(-r) * 25

            # 先画半透明圆点
            plt.scatter(img_x, img_y, facecolors=face_color, edgecolors=edge_color, s=100)

            # 再画半透明箭头
            plt.arrow(img_x, img_y, dx, dy,
                      head_width=15, head_length=20,
                      fc=face_color, ec=edge_color,
                      length_includes_head=True,
                      alpha=0.7)  # 设置整体透明度
            drawn_points.append((img_x, img_y))  # 记录已绘制的点

    # 添加图例
    plt.gca().set_facecolor('none')
    plt.legend(handles=[black_patch, blue_patch], loc='upper left')

    # 确保保存目录存在
    os.makedirs(os.path.join(cfg.result_path, 'figs'), exist_ok=True)
    plt.savefig(os.path.join(cfg.result_path, f"figs/{frame_str}_3_stack_top_view.png"), dpi=300,
