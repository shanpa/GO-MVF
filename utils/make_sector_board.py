...
def get_xzangle(paths=[r'res\out_002282.png.monoloco_test_CSRD.json'] * 2):
...
def get_bbox(paths=[r'res\out_002282.png.monoloco_test_CSRD.json'] * 2):
    dic_out_list = get_dic_out_list(paths)
    box_preds = []
...
def relative_point2index(point, board, ratio=0, mode='center'):
    ...
    delta_x = center_point[0] + x_part  # 图像原点为左上角，y轴向下为正方向
...
def get_grid_and_whole_board(length, width=40, height=40):
    ...
    # board.fill(-0.001)
    board.fill(0)
    return X, Y, board  # X每行都是(-width, width)，Y每列都是(-height, height)
...
class Generator(object):
    ...
    def get_heatmap_from_xzangle_id(self, xzangle_list, ids, if_cropped=True, adding_border=False):
        ...
        X, Y, board = get_grid_and_whole_board(self.length, 30, 30)
        board = torch.as_tensor(board)
        figure_stride = X[0][1] - X[0][0]  # 每一个小方块的边长
        self.board = board
        self.board123()  # self.board的维度变成[3, x, x]，即self.board在第1个维度上重复3次
        ...
        for i, xzangle in enumerate(xzangle_list):
            ...
            try:
                ...
                print('make_sector_board.py: error in id: ', ids[i])
    ...
    def fill_board_by_id(self, board, sector_cache, start_point, length, start_angle, figure_stride, if_filter=True,
        ...
        def put_down():
            epsilon = 50
            mask = assembly > epsilon
            # print('start_index: ', start_index, 'id: ', id)
            for i in range(3):
        ...
        angle = (270 - (start_angle + 90) + 90) % 360
        # angle = xzangle[2]
        angle = int(angle) if angle > 0 else int(360 + angle)  # 角度限制在0~360度之间
        start_index = relative_point2index(start_point, board, figure_stride)  # 检查这行为什么出现负数
        start_index = (int(start_index[1]), int(start_index[0]))
        ...
        if not if_filter:
            ...
            val_epsilon = 10
            num_epsilon = 30
            roi = (roi > val_epsilon).view(3, -1)  # (3, length, length) --> (3, length*length)
